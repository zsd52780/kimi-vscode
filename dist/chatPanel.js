...
const kimiClient_1 = require("./kimiClient");
...
class ChatPanel {
    ...
    constructor(context) {
        ...
        this.panel = vscode.window.createWebviewPanel('kimiK3.chat', 'Kimi K3', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    async doSend(messages, modelOverride, effortOverride) {
        ...
        const baseUrl = cfg.get('baseUrl') ?? DEFAULT_BASE_URL;
        const model = modelOverride || cfg.get('model') || DEFAULT_MODEL;
        const isK3 = model === 'kimi-k3' || model === 'kimi-k3[1m]';
        const reasoningEffort = isK3
            ? (effortOverride || cfg.get('reasoningEffort') || 'max')
            : undefined;
        ...
        try {
            await (0, kimiClient_1.streamChat)({ apiKey, baseUrl, model, reasoningEffort, maxTokens }, fullMessages, signal, {
            ...
            else if (error instanceof kimiClient_1.KimiApiError) {
